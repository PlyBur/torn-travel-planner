...
import { requireCurrentAppUser, getOrCreateSyncState } from "@/lib/current-user";
...
async function fetchFilteredLogs(apiKey: string, logType: number, from: number) {
  ...
  throw new Error(`Torn rate limit persisted after 5 retries for log ${logType}.`);
}
...
async function backfillLogType(options: {
  apiKey: string;
  userId: string;
  logType: number;
  fromUnix: number;
  ...
  let from = options.fromUnix;
  let pagesFetched = 0;
  let scannedLogs = 0;
  let savedRecords = 0;

  while (pagesFetched < options.maxPages) {
    const logs = await fetchFilteredLogs(options.apiKey, options.logType, from);
    const entries = Object.entries(logs) as [string, TornLog][];

    if (entries.length === 0) break;

    pagesFetched += 1;
    let latestTimestampOnPage = 0;
    ...
    for (const [key, log] of entries) {
      ...
      if (log.timestamp > latestTimestampOnPage) {
        latestTimestampOnPage = log.timestamp;
      }
    ...
    if (!latestTimestampOnPage) break;

    from = latestTimestampOnPage + 1;

    if (from >= options.toUnix) break;
...
export async function GET(request: Request) {
  try {
    ...
    await getOrCreateSyncState(user.id);

    const { searchParams } = new URL(request.url);

    const days = Number(searchParams.get("days") ?? user.defaultBackfillDays ?? 30);
    const maxPages = Number(
      searchParams.get("pages") ?? user.defaultBackfillPages ?? 25
    );
    const delayMs = Number(searchParams.get("delayMs") ?? user.defaultDelayMs ?? 2500);
    ...
    for (const logType of TARGET_LOG_TYPES) {
      const result = await backfillLogType({
        apiKey: user.apiKey!,
        userId: user.id,
        logType,
        fromUnix,
    ...
    const totalScannedLogs = results.reduce(
      (sum, result) => sum + result.scannedLogs,
      0
    );

    const totalPagesFetched = results.reduce(
      (sum, result) => sum + result.pagesFetched,
      0
    );

    const totalSavedRecords = results.reduce(
      (sum, result) => sum + result.savedRecords,
      0
    );
    ...
    await prisma.syncState.update({
      ...
      data: {
        ...
        backfillDays: days,
        backfillFromDate: new Date(fromUnix * 1000),
        backfillToDate: new Date(toUnix * 1000),
        lastBackfillAt: new Date(),
        backfillScannedLogs: totalScannedLogs,
      },
    });
    ...
    return NextResponse.json(
      serialize({
        ...
        mode: "initial-user-backfill",
        days,
        fromUnix,
        toUnix,
        pagesFetched: totalPagesFetched,
        scannedLogs: totalScannedLogs,
