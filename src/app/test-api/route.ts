...
async function syncLogType(options: {
  ...
  const logs = await fetchFilteredLogs(options.apiKey, options.logType, options.fromUnix);
...
export async function GET() {
  try {
    ...
    const syncState = await getOrCreateSyncState(user.id);
